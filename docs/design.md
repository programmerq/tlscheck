...
- **Logging**: capture target host, resolved IP, SNI sent, ALPN list, negotiated protocol, leaf
  certificate fingerprint/subject, upgrade path details, and proxy metadata when applicable. Each
  probe result includes a `certificate_chain` field containing an ordered list of certificate
